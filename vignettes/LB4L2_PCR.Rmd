...
acc <- autoplot(IV, DV = "accuracy") + 
  geom_point(aes(y = acc), data = filter(IV_pred, accuracy ==1),
             position = position_jitter(.05), shape = 18, size = 3) + 
  ggtitle("PCR vs. Obs.")
print(acc)
RT <- autoplot(IV, DV = "RT") +  
  geom_point(aes(y = RT), data = rename(IV_pred, final_acc = accuracy),
             position = position_jitter(.05), shape = 18, size = 3) + 
  ggtitle("PCR vs. Obs.")
print(RT)
```
