...
IV_obs <- summary(final, level = "subject") %>%
  ...
  summarise(RTs = list(round(RT,1))) %>%
  inner_join(x = select(summary(final, level = params$mode), group:mean_RT),
             y = .,
             by = c("group", "practice", "OCpractice", "final_acc"))

joint_obs <- summary(final, given_practice = 1, given_data = practice) %>%
  filter(p != 0) %>%
  select(group:final_acc, contains("RT")) %>%
  group_by(group, sameCue, practice1acc, final_acc) %>%
  summarise(RTs = list(matrix(c(practice1RT, RT), ncol = 2))) %>%
  inner_join(x = select(summary(final, level = "group",
                                given_practice = 1, given_data = practice),
                        group:mean_RT),
             y = .,
             by = c("group", "sameCue", "practice1acc", "final_acc")) %>%
  rowwise() %>%
  mutate(RTs = if(final_acc == 1 & practice1acc == 1) {list(RTs)} else {list(NULL)})
...
param_search <- function(theta, PCR, parameter_sets) {
  ...
  if (params$routine =="minimize") {
    results <- optimx(par = p,
                      ...
                      prediction_fcn = LB4L2_PCR, # for fn
                      error_fcn = LB4L2_PCR_erf,# for fn
                      groups = globalenv()$groups, # for objective_fn
                      observations = list(final = filter(IV_obs, subject==index), # for error_fcn
                                          joint = filter(joint_obs, subject==index)),
                      likelihood = names(parameter_sets[1])) # for error_fcn
  } else {
    error <- fitPCR(free_parameters = p, 
                    PCRparams_obj = PCR,
                    prediction_fcn = LB4L2_PCR, 
                    error_fcn = LB4L2_PCR_erf,
                    groups = globalenv()$groups, # for objective_fn
                    observations = list(final = filter(IV_obs, subject==index), # for error_fcn
                                        joint = filter(joint_obs, subject==index)),
                    likelihood = names(parameter_sets[1])) # for error_fcn
...
IVplot_data <- select(error$IV[[1]], -subject, -RTs) %>%
...
jointplot_data <- select(error$Joint[[1]], -subject, -RTs)  %>%
