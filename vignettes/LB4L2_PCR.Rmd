...
```{r data}
final <- LB4L2_final %>%
  mutate(acc = ifelse((RT < .2 & !is.na(RT)) | (RT > 8  & !is.na(RT)), 0, acc))
practice <- LB4L2_tp %>%
  mutate(acc = ifelse((RT < .2 & !is.na(RT)) | (RT > 8  & !is.na(RT)), 0, acc))

sub_RT <- summary(as_LB4L(final), level = "subject") %>%
  filter(practice != "T", OCpractice != "T", final_acc ==1, probability !=0) %>%
...
IV_obs <- summary(as_LB4L(final), level = params$mode) %>%
  select(group:OCpractice, final_acc, acc = probability_mean, RT = RT_mean) %>%
  inner_join(sub_RT)
rm(sub_RT)

joint_obs_sub <- summary(as_LB4L(final, practice_tests = 2), level = "subject",
                         given_practice = 1,  given_data = practice) %>%
...
joint_obs <- summary(as_LB4L(final, practice_tests = 2), level = "group",
                     given_practice = 1,  given_data = practice) %>%
  select(group:final_acc, acc = probability_mean, RT = RT_mean) %>%
  inner_join(joint_obs_sub, by = c("group", "sameCue", "practice1acc", "final_acc")) %>%
  rename(practice = practice1acc, final = final_acc) %>%
  rowwise() %>%
  mutate(rawRTs = if(final == 1 & practice == 1) {list(rawRTs)} else {list(NULL)},
         RT = NULL)
rm(joint_obs_sub)
...
```{r model_parameters}
acc_params <- c("ER","LR","FR","FR2","TR","space")
RT_params <- c("Tmin","lambda")
theta <- list(ER=.56, LR=.2, FR=.15, FR2=.3, TR=.02, space = .02,
                   Tmax = 8, Tmin = 0.2, lambda = .25)
parameter_sets <- switch(params$likelihood,
                         accuracy = list(acc_params),
                         RT = list(RT_params),
                         all = list(acc_params, RT_params, c(acc_params, RT_params)))
...
if (params$mode == "subject") {
  ...
  model <- replicate(n = length(unique(IV_obs$subject)),
                     expr = initPCR(params = theta, distribution = "binomial",
  ...
  groups <- 2
  model <- initPCR(params = theta, distribution = "binomial", nItems = 20,
...
param_search <- function(theta, PCR, parameter_sets) {
  ...
  index = globalenv()$j
  results <- optimx(par = unlist(theta[ parameter_sets[[1]] ]), 
                    fn = fitPCR,
                    method="Nelder-Mead",
                    itnmax = globalenv()$iterations,
                    ...
                    likelihood = params$likelihood) # for error_fcn
  ...
  while (done < length(parameter_sets)) {
    ...
    PCR = update(PCR, new)
    next_results <- param_search(theta, PCR, parameter_sets[done+1])
...
print(xtable(cbind(best_params, D = 2*fit[1,"value"]), digits=3),
      type = "html", include.rownames=FALSE, caption.placement="top")
```

## Plots
```{r plots}
IV_pred <- best_preds$final %>%
    select(group, practice, OCpractice, accuracy,
           acc = proportion, RT = medianRT, rawRTs = rawRTs) %>%
    rowwise() %>%
    mutate(rawRTs = list(round(rawRTs,1)),
           group = if(group == "group1") {"immediate"} else {"delay"})
IV <- summary(as_LB4L(final), level = params$mode) 

acc <- autoplot(IV, DV = "accuracy") + 
  geom_point(aes(x = group, y = acc, color = interaction(practice, OCpractice)),
             data = filter(IV_pred, accuracy ==1),
             position = position_jitter(.05),
             inherit.aes = FALSE, shape = 18, size = 3) + 
  ggtitle("PCR vs. Obs.")
print(acc)
RT <- autoplot(IV, DV = "RT") +  
  geom_point(aes(x = group, y = RT, color = interaction(practice, OCpractice)),
             data = rename(IV_pred, final_acc = accuracy),
             position = position_jitter(.05),
             inherit.aes = FALSE, shape = 18, size = 3) + 
  ggtitle("PCR vs. Obs.")
print(RT)
