...
for (DV in c("RT_mean", "RT_median", "logRT_mean")) {
  ...
  RT_revOrder_anova <- aov_car(model_formula,
                               data = filter(bySubjects_cond_revOrder_noMissing,
                                             revOrder < min(max_complete$max_position)) %>%
...
```{r follow_up_ANOVA, include=FALSE}
...
pairs(lsmeans(RT_order_anova, ~ class), adjust = "bonferroni")
```
