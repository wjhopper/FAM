...
meanRT <- reversed %>%
  ...
  summarise(meanRT = mean(RT),
            count = n())
ggplot(data = reversed, 
       aes(x= revOrder,y=RT)) +
  geom_point(position = position_jitter(width = .2)) + 
  geom_point(aes(y=meanRT), data=meanRT,
             color="red") +
  geom_line(aes(y=meanRT), data=meanRT,
             color="red") + 
  geom_text(aes(label = count, y=-5),data=meanRT) +
  scale_x_discrete("Item Outout",
