...
gamma_cor_bySubject_List <- ranks %>%
  ...
  summarise(gamma = rococo(id[class=="np"],
                           id[class=="tp"])) %>%
  left_join(Ncorrect_diff,
            by = c("subject", "cond_list"))
```

```{r gamma_cor_figures, fig.height=4, fig.width=5, results="asis", fig.caption=c("A","B")}
...
plot_gamma_cor_bySubject_List <- ggplot(gamma_cor_bySubject_List,
                                        aes(x=diff,y=gamma)) +
  geom_point() +
  xlab("Difference in Items Recalled") + 
  ylab(expression(paste("Goodman–Kruskal ", gamma, " Coefficient")))
print(plot_gamma_cor_bySubject_List)
```
