...
```{r gamma_correlation, fig.height=4, fig.width=5, results="asis"}
ranks <- corrects %>%
...
gamma_correlations <- ranks %>%
  group_by(subject, cond_list) %>%
  do(bind_cols(distinct(.[c("subject","cond_list")]),
               as_data_frame(t(rank_agreements(x = .$rank[.$class=="np"],
                                               y = .$rank[.$class=="tp"])))
               ))  %>%
  group_by(subject) %>%
...
hist(gamma_correlations$gamma, xlab="gamma")
pander(t.test(gamma_correlations$gamma_z),
       style="rmarkdown")
```
