...
agg <- filter(CFR_allSs,
  ...
  ungroup() %>% 
  complete(subject,class,order, cond_list, fill=list(score=0)) %>%group_by(class,order) %>% 
...
aggAccPlot <- ggplot(data =agg,
                     aes(x=factor(order), y=acc,  group=class, linetype=class)) +
  geom_point(size = 2) +
  geom_line(size = .75) +
  ...
  scale_linetype_discrete(name="Condition", labels = c('No Practice',
                                              'Restudy',
                                              'Test Practice'))  + 
  ggtitle("Data vs. PCR: Output Accuracy")

aggRTPlot <- ggplot(data = agg,
                    aes(x=factor(order), y=RT, group=class, linetype=class)) +
  geom_point(size =2) +
  geom_line(size =.75) +
