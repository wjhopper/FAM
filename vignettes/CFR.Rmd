...
RT_by_cond_revOrder_ANOVA_data <- filter(bySubjects_cond_revOrder_noMissing,
                                         revOrder < min(max_complete$max_position)) %>%
                                         mutate_at(.cols = c("subject","class","revOrder"),
                                                   .funs = factor
                                         )
...
for (DV in c("RT_mean", "RT_median", "logRT_mean")) {
  ...
  RT_revOrder_anova <- aov_car(model_formula,
                               data = RT_by_cond_revOrder_ANOVA_data,
                               anova_table = list(es = 'pes')
                               )
  ...
  for (x in strata) {
    ...
    qqline(y)
    SW_tests[[x]] <- tidy(shapiro.test(y))
  }
  SW_tests_table <- bind_rows(SW_tests, .id = "strata")
  colnames(SW_tests_table)[2] <- 'W'
  pander(SW_tests_table[1:3],
         caption = as.character(SW_tests[[1]][['method']]),
         style="rmarkdown")
...
## RT ANOVA Contrasts
```{r follow_up_contrasts, message=FALSE}
pander(summary(pairs(lsmeans(RT_revOrder_anova, ~ class), adjust = "bonferroni")),
       caption = "P value adjustment: bonferroni method for 3 tests")
```
...
np_tp_only_anova <- aov_car(model_formula,
                            data = filter(RT_by_cond_revOrder_ANOVA_data,
                                          class %in% c("np", "tp")),
                            anova_table = list(es = 'pes'))
...
tp_sp_only_anova <- aov_car(model_formula,
                            data = filter(RT_by_cond_revOrder_ANOVA_data,
                                          class %in% c("tp", "sp")),
                            anova_table = list(es = 'pes'))
pander(nice(tp_sp_only_anova), "Test Practice vs Restudy")
```

### Forward Order
```{r first_seven}
RT_order_anova <- aov_car(logRT_mean ~ class*output_order + Error(subject/(class*output_order)),
                          data = filter(bySubjects_cond_order_noMissing,
                                        output_order > 1) %>%
                                 mutate_at(.cols = c("subject","class","output_order"),
                                           .funs = factor
                                           ),
                          anova_table = list(es = 'pes'))
pander(nice(RT_order_anova), "First 7 ANOVA")
...
RT_breaks <- seq(round(min(byCond_revOrder_noMissing$logRT_mean),0),
                 ...
                 by=.5)
byCond_revOrder_RTplot <- ggplot(filter(byCond_revOrder_noMissing,
                                        revOrder < min(max_complete$max_position)),
